...
#
# 이전에는 리사이즈가 전혀 없어 폰카 원본(4000px급)이 그대로 올라갔다.
# 비전 토큰은 해상도에 비례하므로 비용의 대부분이 여기서 발생했다.
#
# 손글씨 판독에는 1536px 이면 충분하다. 그보다 키워도 인식률이 눈에 띄게
# 좋아지지 않으면서 토큰만 늘어난다. 실험이 필요하면 환경변수로 조정한다.
...
def image_content_part(image_b64: str, detail: str = "high") -> dict:
    ...
    detail 을 명시하지 않으면 고해상도 모드로 처리되어 토큰이 크게 늘어난다.
    구조만 파악하면 되는 작업(분류 등)은 "low" 로 충분하다.
    """
    return {
        "type": "image_url",
        "image_url": {
